 ...
 * US Star Gazers
 *
 * Generates all the functionality needed to display the weather and space info for the site
 *
 * @package US Star Gazers
 *
 * Plugin Name: US Star Gazers
 * Plugin URI: https://github.com/kpirnie/wpplugin-stargazers
...
define('SGUP_VERSION', '1.0.0');

// hold the plugin path
define('SGUP_PATH', __DIR__);

// hold the primary plugin file name
define('SGUP_FILENAME', basename(__FILE__));

// hold the plugin url
defined('SGUP_URI') || define('SGUP_URI', plugin_dir_url(__FILE__));

// include our autoloader if it exists
if (file_exists(SGUP_PATH . '/vendor/autoload.php')) {
    include SGUP_PATH . '/vendor/autoload.php';
}

// fire us up on initialization
add_action('init', function (): void {

    // we can fire up the rest of the plugin's functionality now
    $sgu = new SGUP();
...
if (defined('WP_CLI')) {
    ...
    add_action('cli_init', function (): void {
        ...
        WP_CLI::add_command(
            'sgu historical imagery',
            ['SGU_Sync', 'sync_historical_imagery'],
...
// strip the block supports we do not want on our own blocks
